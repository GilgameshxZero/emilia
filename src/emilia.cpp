	...
	return Rain::Error::consumeThrowable(
		[argc, argv]() {
			...
			std::cout << "emilia " << EMILIA_VERSION_MAJOR << "."
								<< EMILIA_VERSION_MINOR << "."
								<< EMILIA_VERSION_REVISION << "."
								<< EMILIA_VERSION_BUILD << " / rain "
								<< RAIN_VERSION_MAJOR << "."
								<< RAIN_VERSION_MINOR << "."
			...
			if (showHelp) {
				...
				std::cout << "Command-line options (default "
										 "specified in parenthesis):\n"
									<< "--help, -h (off): Display this help "
										 "message and exit.\n"
									<< "--http-port (\"0\"): Alter the "
										 "listening port of the HTTP "
										 "server.\n"
									<< "--smtp-port (\"0\"): Alter the "
										 "listening port of the SMTP "
										 "server.\n"
									<< "--http-password (\"\"): Password for "
										 "authenticated routes.\n"
									<< "--smtp-forward (\"\"): If specified, "
			...
			// Launch both servers. HTTP server requires SMTP
			// server state.
			auto newSmtpServer = [&]() {
			...
			std::unique_ptr<Emilia::Smtp::Server> smtpServer(
				newSmtpServer());
			std::cout << "SMTP server listening on "
			...
			std::unique_ptr<Emilia::Http::Server> httpServer(
				newHttpServer());
			...
			while (true) {
				...
				if (command == "help") {
					std::cout
						...
						<< "start smtp: If SMTP server stopped, start "
							 "it.\n"
						<< "start http: If HTTP server stopped, start "
							 "it.\n"
						<< "modify smtp: Modify SMTP server port. "
							 "Restart required.\n"
						<< "modify http: Modify HTTP server port. "
							 "Restart required.\n"
						<< "clear outbox: Clear non-PENDING outbox "
							 "history.\n"
						<< "trigger outbox: Forces a send of any "
							 "PENDING envelopes, even "
							 "if they are scheduled in the future.\n"
						<< "modify forward: Modify SMTP forwarding "
							 "address.\n"
						<< "modify password: Modify SMTP "
							 "authentication password.\n"
						<< "toggle echo: Toggles echoing of all parsed "
							 "requests/responses "
							 "to "
							 "the command-line.\n"
					...
					std::cout << "Triggered outbox send event."
										<< std::endl;
