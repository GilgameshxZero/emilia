...
namespace Rain {
	...
	unsigned int checksumFileCRC32(std::string file) {
		static const unsigned int crc32Tab[] = { /* CRC polynomial 0xedb88320 */
		...
		for (std::size_t a = 0; in; a += BUFFER_LENGTH) {
			in.read(buffer, BUFFER_LENGTH);
			for (int b = 0; b < in.gcount(); b++) {
				hash = (crc32Tab[((hash) ^ (buffer[b])) & 0xff] ^ ((hash) >> 8));
			}
		}
