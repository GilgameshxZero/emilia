...
// Near-constant time amortized union and find.
//
// Implements path compression and union by rank.
class DisjointSetUnion {
	private:
	// A pair of (is_root, X). If node is root, X stores the
	// size of the cluster. Otherwise, X stores the index of
	// the node’s parent.
	...
	std::size_t find(std::size_t const i) const {
		...
		return this->nodes[i].second =
						 this->find(this->nodes[i].second);
	}
...
AR<AR<short, MAX_N>, MAX_N> L, U, &L_ = L, &U_ = U, R__,
																	&R_ = L;
AR<string, MAX_N> G;
AR<short, MAX_N> S__;
		...
		RF(i, N - 1, -1) {
			...
			while (!R__radix[2 - t].empty()) {
				...
				if (i_ > 0) {
					...
					// Pseudo-join the first corner by setting its
					// answer to negative.
					S__[S_.find(i_)] = -1;
				}
