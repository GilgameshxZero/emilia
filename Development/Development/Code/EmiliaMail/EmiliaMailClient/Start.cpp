...
namespace Monochrome3 {
	namespace EMTSMTPClient {
		int start() {
			...
			Rain::readParameterFile("config.ini", config); //don't need all the parameters just yet, but read it anyway just for readConfig
			...
			if (config["readConfig"] != "yes") {
				...
				configFIn.open("config.ini", std::ios::binary);
				configSS = &configFIn;
			}
