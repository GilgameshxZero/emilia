...
namespace Rain {
	typedef int(*RecvHandlerFunc) (void *);
	...
	class WSA2RecvFuncParam {
		public:
		WSA2RecvFuncParam();
		WSA2RecvFuncParam(SOCKET *socket, std::string *message, int bufLen, void *funcparam, RecvHandlerFunc onProcessMessage, WSA2RecvInitFunc onRecvInit, WSA2RecvExitFunc onRecvExit);
	...
	HANDLE createRecvThread(
		...
		RecvHandlerFunc OnProcessMessage = NULL,
